...
// define flags
  ...
  constexpr bool writeSignal = true;
	...
	std::string normal_bias_grd = "normal_bias_32_0000_lgad_des.grd";
	std::string normal_bias_dat = "normal_bias_32_0000_lgad_des.dat";

	MediumSilicon si;
	ComponentTcad2d cmp;
//  cmp.EnableDebugging(); // open debug mode
  cmp.Initialise(normal_bias_grd, normal_bias_dat);
...
// weighting field from tcad file
	...
	double dv = 1;
	dwp_1.Initialise(normal_bias_grd, normal_bias_dat);
	dwp_2.Initialise(normal_bias_grd, normal_bias_dat);
	dwp_3.Initialise(normal_bias_grd, normal_bias_dat);
	dwp_4.Initialise(normal_bias_grd, normal_bias_dat);
	dwp_5.Initialise(normal_bias_grd, normal_bias_dat);
	dwp_1.SetWeightingField(normal_bias_dat, "dynamic_n32_0000_lgad_des.dat", dv, "pad1-delayed");
	dwp_2.SetWeightingField(normal_bias_dat, "dynamic_n33_0000_lgad_des.dat", dv, "pad2-delayed");
	dwp_3.SetWeightingField(normal_bias_dat, "dynamic_n34_0000_lgad_des.dat", dv, "pad3-delayed");
	dwp_4.SetWeightingField(normal_bias_dat, "dynamic_n35_0000_lgad_des.dat", dv, "pad4-delayed");
	dwp_5.SetWeightingField(normal_bias_dat, "dynamic_n36_0000_lgad_des.dat", dv, "pad5-delayed");
	
	double start = 4.e-3; // start from 4ps
	double end = 5.; // end at 5ns
	double q = pow(end / start, 1./50);
	std::vector<double> dy_times;
	std::string prefix_1 = "dynamic_n32_";
	std::string prefix_2 = "dynamic_n33_";
	std::string prefix_3 = "dynamic_n34_";
	std::string prefix_4 = "dynamic_n35_";
	std::string prefix_5 = "dynamic_n36_";
	...
	for (int i = 0; i < 51; i++) {
		...
		dwp_1.SetWeightingField(normal_bias_dat, filename, dv, t, "pad1-delayed");
	...
	for (int i = 0; i < 51; i++) {
		...
		dwp_2.SetWeightingField(normal_bias_dat, filename, dv, t, "pad2-delayed");
	...
	for (int i = 0; i < 51; i++) {
		...
		dwp_3.SetWeightingField(normal_bias_dat, filename, dv, t, "pad3-delayed");
	...
	for (int i = 0; i < 51; i++) {
		...
		dwp_4.SetWeightingField(normal_bias_dat, filename, dv, t, "pad4-delayed");
	...
	for (int i = 0; i < 51; i++) {
		...
		dwp_5.SetWeightingField(normal_bias_dat, filename, dv, t, "pad5-delayed");
...
// signal simulation
	...
	const unsigned int nEvents = 50;

	for (unsigned int k = 0; k < (numofpos + 1); ++k) {
	for (unsigned int j = 0; j < nEvents; ++j) {
		sensor.ClearSignal();
		double y0 = 0.5 * (left + right) - numofpos * deltay + k * 2 * deltay; // for train data
//		double y0 = 0.5 * (left + right) + (RndmUniform() - 0.5) * (gap + side) * 4; // for test data
//		double y0 = 0.5 * (left + right); // for first try
		output_y_real << y0 << "\n";
		std::cout << "y0 = " << y0 << "\n";
		double t0 = 0.1;
...
// write time info		
		...
		if (writeTime) {
			...
			for (unsigned int i = 0; i < nSignalBins; ++i) {
				const double f = sensor.GetSignal("cathode", i);
				const double t = (i + 0.5) * tStep;
				if (f < min) {
					min = f;
					min_time = t;
